...
function transit_surface() {
  ...
  object_distance =
    object_distance *
    ((to_index * Math.cos(-rang)) / (from_index * Math.cos(axis_slope_angle)));
...
function RunBenchmark(niter, log) {
  ...
  for (var itercount = 0; itercount < niter; itercount++) {
    ...
    aberr_osc =
      1.0 - (od_sa[1][0] * od_sa[1][1]) / (Math.sin(od_sa[0][1]) * od_sa[0][0]);
  ...
  show("Microseconds per iteration: " + ((mstime / niter) * 1000.0).toFixed(4));

  let timeMessage = "Elapsed time in seconds: " + (mstime / 1000).toFixed(3);
...
exports.handler = async (event) => {
  ...
  if (event.body) {
    ...
    if (ele.itercount) {
      const msg = RunBenchmark(ele.itercount, false);
